...
args['nlp_path'] = os.path.join(data_root, 'additional_info/kmeans62/nlp_annotations')
...
#         # Reason feats --------------------------
        print("----- reason ------")
        time_rsn_map = ori_int_df.pivot(index = 'video_time', columns='user_id', values = 'reasoning_labeled')
...
#                 print("short frames: ", video, pedID, min_end - max_start)
            try:
...
database_name = 'database_' + time.strftime("%d%b%Y-%Hh%Mm%Ss") + '_kmeans.pkl'
...
#                     print(i, 'ori_rsn empty: ', sample['original_reason'][i])
            ...
            if len(mis_match_list) > 0:
                ...
                del overlap_db[v][p]['reason_feats'][mis_match_list[0]: mis_match_list[-1]+1]
...
database_name = 'database_' + time.strftime("%d%b%Y-%Hh%Mm%Ss") + '_kmeans_overlap.pkl'
if not os.path.exists(os.path.join(args['save_path'])):
    os.makedirs(os.path.join(args['save_path']))
with open(os.path.join(args['save_path'], database_name), 'wb') as fid:
